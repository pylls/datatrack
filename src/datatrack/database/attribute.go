...
func AddAttribute(a model.Attribute) (err error) {
	return DB.Batch(func(tx *bolt.Tx) error {
		...
		// update type->id map
		err = appendValueInMap(a.Type, a.ID, "type2id", abmap)
		if err != nil {
			return err
		}

		// update type->value map
		err = appendValueInMap(a.Type, a.Value, "type2value", abmap)
		if err != nil {
			return err
		}
...
func AddAttributes(as []model.Attribute, wg *sync.WaitGroup, errChan chan error) {
	...
	err := DB.Batch(func(tx *bolt.Tx) error {
		...
		// write maps
		err = writeMap("type2id", type2id, abmap)
		if err != nil {
			return err
		}
		err = writeMap("type2value", type2value, abmap)
		if err != nil {
			return err
		}
